...
static const u8 sText_CannotThrowBall[] = _("Cannot throw a ball in this location anymore...\p");
...
void BufferStringBattle(enum StringID stringID, u32 battler)
{
    ...
    case STRINGID_USEDMOVE: // Pokémon used a move msg
        if (gBattleMsgDataPtr->currentMove >= MOVES_COUNT && !IsZMove(gBattleMsgDataPtr->currentMove) && !IsMaxMove(gBattleMsgDataPtr->currentMove))
    ...
    case STRINGID_NUZLOCKELOST:
        ...
        break;
    case STRINGID_NUZLOCKECANNOTTHROW:
        stringPtr = sText_CannotThrowBall;
        break;
...
static const u8 *BattleStringGetPlayerName(u8 *text, u8 battler)
{
    ...
    case B_POSITION_PLAYER_RIGHT:
        if (((gBattleTypeFlags & BATTLE_TYPE_RECORDED) && !(gBattleTypeFlags & (BATTLE_TYPE_MULTI | BATTLE_TYPE_INGAME_PARTNER))) || gTestRunnerEnabled)
        {
            toCpy = gLinkPlayers[0].name;
        }
    ...
    while (*src != EOS)
    {
        ...
        if (*src == PLACEHOLDER_BEGIN)
        {
            ...
            case B_TXT_LAST_MOVE: // originally used move name
                if (gBattleMsgDataPtr->originallyUsedMove >= MOVES_COUNT && !IsZMove(gBattleMsgDataPtr->currentMove) && !IsMaxMove(gBattleMsgDataPtr->currentMove))
                    toCpy = gTypesInfo[gBattleStruct->stringMoveType].generic;
                else
...
{
    BattlePutTextOnWindowWithColor(text, windowId, 0);
}

void BattlePutTextOnWindowWithColor(const u8 *text, u8 windowId, u32 color)
{
    ...
    printerTemplate.fgColor = (color == 0) ? textInfo[windowId].fgColor : color;
